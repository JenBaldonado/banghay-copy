...
/* ----------------------------------Global Variables--------------------------- */
var CLIENT_ID =
  "1078263943954-qco2ieimo7rbdjg2uim2hh6udf550c87.apps.googleusercontent.com";
var API_KEY = "AIzaSyCMeouW-JlUFHwH7kJIkjkg2fiiNW1NGNk";
var DISCOVERY_DOCS = [
...
// check for a Banghay Folder in google drive
function checkFolder() {
  gapi.client.drive.files
    .list({
      q: 'name = "Banghay Folder"',
    })
...
function upload() {
  var text = document.querySelector(".share-textarea");
  var grade = document.getElementById("grade-one");
  if (text.value != "") {
    ...
    // set file metadata
    var metadata = {
      // get first two words from the input text and set as file name
      name: twoWords + ".txt",
      mimeType: "plain/text",
...
function readEditDownload(v, condition) {
  ...
  gapi.client.drive.files
    ...
    .then(function (res) {
      ...
      if (condition == "read") {
        v.innerHTML = "Read";
        document.querySelector(".preview-textarea").value = res.body;
        ...
        //updateBtn.innerHTML = 'Update'; // needs revision When the user clicks edit the update button will be visible
        // we will make the update function later
        updateBtn.setAttribute("onClick", "update()");
        document
          .querySelector(".preview-textarea")
...
function update() {
  ...
  fetch(url, {
    ...
    .then((value) => {
      ...
      //updateBtn.innerHTML = 'Backup'; //needs revision
